...
class App(ctk.CTk):
    ...
    def criar_interface(self):
        ...
        for col in colunas:
            ...
            if col == "Turmas":
                ...
                f = ctk.CTkFrame(frame)
                f.pack(padx=10, pady=5, anchor="w")
                for opt in ["Agendado", "Realizado"]:
                    ctk.CTkRadioButton(f, text=opt, value=opt, variable=var).pack(side="left", padx=5)
                self.entries[col] = var

            elif col == "Ano":
                anos = [str(ano) for ano in range(2020, 2036)]
                combo = ctk.CTkComboBox(frame, values=anos, width=200)
                combo.pack(padx=10, pady=5)
                self.entries[col] = combo

            elif col == "Número participantes":
                nums = [str(i) for i in range(1, 501)]
                combo = ctk.CTkComboBox(frame, values=nums, width=200)
                combo.pack(padx=10, pady=5)
                self.entries[col] = combo
    ...
    def salvar(self):
        ...
        if self.foto_path:
            ...
            nome = os.path.basename(self.foto_path)
    ...
    def gerar_relatorio(self):
        ...
        ws = wb.active
        colunas_rel = [
            {"busca": "conte", "titulo": "Os conteúdos e temas abordados foram interessantes e contribuíram para o conhecimento."},
            {"busca": "visita", "titulo": "O que achou da visita do RioPretoPrev Itinerante no seu local de trabalho?"},
            {"busca": "tempo dedicado", "titulo": "Tempo dedicado ao diálogo/exposição foi adequado."}
        ]
        ...
        for info in colunas_rel:
            ...
            for k, v in cont.items(): relatorio += f"{k}: {v}\n"
            relatorio += "\n--------------------------\n\n"

        with open("relatorio.txt", "w", encoding="utf-8") as f:
            f.write(relatorio)
        messagebox.showinfo("Sucesso", "Relatório TXT gerado com sucesso!")
